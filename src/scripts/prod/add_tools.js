...
const readExcelData = () => {
  ...
  const workbook = XLSX.readFile(
    "/Users/pushpitbhardwaj/Desktop/eai/codebase/everythingai/output.xlsx"
  );
    ...
    if (obj.A) {
      ...
      for (const [key, value] of Object.entries(obj)) {
        if (key === "A") {
          ...
          convertedObj["youtubeDemoVideoLink"] =
          ...
          convertedObj["twitter"] =
            value && value !== "NA" ? value?.trim() : "";
        } else if (key === "P") {
          convertedObj["instagram"] =
            value && value !== "NA" ? value?.trim() : "";
        } else if (key === "Q") {
          convertedObj["linkedin"] =
            value && value !== "NA" ? value?.trim() : "";
        } else if (key === "R") {
          convertedObj["youtube"] =
            value && value !== "NA" ? value?.trim() : "";
        } else {
          if (key === "S") {
  ...
  try {
    ...
    data.forEach(async (tool) => {
      ...
      if (!categories.hasOwnProperty(category)) {
        console.log(
          `Category "${category}" not found in the categories object for tool "${tool.name}". Using Industries instead.`
        );
      ...
      for (const option of pricingOptions) {
        if (
          tool.pricing.name == option.name &&
          tool.pricing.meta == option.meta
        ) {
      ...
      if (!matchedPricingOption) {
        console.log(
          `No matching pricing option found for tool "${
            tool.name
          }". ${JSON.stringify(tool.pricing)}. Using Freemium instead.`
        );
      ...
      if (!tool.url.startsWith("http://") && !tool.url.startsWith("https://")) {
        console.log(
          `Invalid URL format for tool "${tool.name}". The URL must start with "http://" or "https://"`
        );
        // await deleteDatabase();
      }
    });
    ...
    for (const tool of data) {
      ...
      const toolAlreadyPresent = allTools.find(
        (_tool) => _tool.slug === tool.slug
      );
      if (toolAlreadyPresent) {
        console.log(
          `Tool: ${tool.name} is already present in the database. Skipping...`
        );
        continue;
      }
      ...
      if (categoryName && categories.hasOwnProperty(categoryName)) {
        ...
        if (subCategoryName) {
          ...
          if (subCategory) {
            tool.subCategory = mongoose.Types.ObjectId(
              subCategory._id.toString()
            );
          } else {
            console.error(
              `Subcategory "${tool.subCategory}" not found in category "${category.name}" for tool: ${tool.name}`
            );
      ...
      if (name) {
        const pricing = await allPricings.find(
          (pricingOption) =>
            pricingOption.name === name && pricingOption.meta === meta
        );
        if (pricing) {
