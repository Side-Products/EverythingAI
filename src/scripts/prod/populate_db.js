  ...
  try {
    ...
    data.forEach(async (tool) => {
      ...
      if (!categories.hasOwnProperty(category)) {
        console.log(
          `Category "${category}" not found in the categories object for tool "${tool.name}"`
        );
        await deleteDatabase();
      }
      if (subCategory !== "" && !categories[category]?.includes(subCategory)) {
        console.log(
          `Mismatch found: Tool "${tool.name}" has a different subCategory "${subCategory}" for category "${category}"`
        );
        await deleteDatabase();
      }
    ...
    const categoryInsertResult = await Category.insertMany(
      Object.keys(categories).map((category) => ({
        name: category,
        image:
          "https://everythingai.s3.amazonaws.com/171ebe4a-2f9b-42cc-aa5f-7e4652705f3f.jpg",
      }))
    );
    ...
    for (const [category, subcategories] of Object.entries(categories)) {
      const categoryObj = categoryInsertResult.find(
    ...
    const subCategoryInsertResult =
      await SubCategory.insertMany(subCategoryDocuments);
    console.log(
      subCategoryInsertResult.length,
      "Subcategories inserted successfully"
    );
    ...
    const collectionIds = collectionInsertResult.map(
    ...
    for (const tool of data) {
      ...
      if (categoryName && categories.hasOwnProperty(categoryName)) {
        ...
        const category = await categoryInsertResult.find(
      ...
      if (name) {
        const pricing = await pricingInsertResult.find(
          (pricingOption) =>
