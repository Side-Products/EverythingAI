...
export const getPurchaseTermsById =
	...
	async (dispatch) => {
		try {
			...
			const { data } = await axios.get(`${origin}/api/purchase-terms/${id}`, config);
...
export const getPurchaseTermsByToolSlug = (req, slug) => async (dispatch) => {
	try {
		...
		const { data } = await axios.get(`${origin}/api/purchase-terms/slug/${slug}`);
		dispatch({
			type: GET_PURCHASE_TERMS_SUCCESS,
			payload: data,
		});
...
export const createPurchaseTerms = (purchaseTermsData, id) => async (dispatch) => {
	try {
		...
		const { data } = await axios.post(`/api/purchase-terms/${id}`, purchaseTermsData, config);
		dispatch({
...
export const updatePurchaseTerms = (purchaseTermsData, id) => async (dispatch) => {
	try {
		...
		const { data } = await axios.put(`/api/purchase-terms/${id}`, purchaseTermsData, config);
