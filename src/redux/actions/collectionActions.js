...
export const createCollection = (collectionData) => async (dispatch) => {
	try {
		...
		const { data } = await axios.post(`/api/collections`, collectionData, config);
...
export const updateCollection = (id, collectionData) => async (dispatch) => {
	try {
		...
		const { data } = await axios.post(`/api/collections/${id}`, collectionData, config);
...
export const getCollectionBySlug = (req, slug) => async (dispatch) => {
	try {
		...
		const { data } = await axios.get(`${origin}/api/collections/find/${slug}`, config);
