	...
	return (
		<Transition show={success.showSuccessBox}>
			...
			>
				<div
					className={
						"fixed bottom-0 right-0 z-[110] lg:w-5/12 md:w-8/12 sm:w-3/4 w-11/12 px-5 py-4 rounded-lg shadow-2xl -translate-x-4 -translate-y-4 break-words " +
						(style == 1
				...
				>
					<p
						className={
							"font-semibold mb-4 " +
							(style == 1 ? "text-base" : style == 2 ? "text-base text-success-500" : style == 3 ? "text-base text-success-500" : "")
						}
					>
						{success.title}
					</p>
					<span className={"block sm:inline " + (style == 1 ? "text-base" : style == 2 ? "text-sm" : style == 3 ? "text-sm" : "")}>
						{success.message}
					</span>
					<div
						onClick={handleClose}
						className={
							...
							(style == 1 ? "w-8 h-8 top-1 right-1" : style == 2 ? "w-6 h-6 top-2 right-2" : style == 3 ? "w-6 h-6 top-2 right-2 group" : "")
						}
					>
						<i
							className={
