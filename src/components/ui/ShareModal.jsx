...
const ShareModal = ({ isOpen, setOpen, tool, shareableDashboardUser, shareMyFavouriteTools }) => {
	...
	useEffect(() => {
		if (shareMyFavouriteTools) {
			if (session && session.user) {
				setCurrentPageLink(window.location.origin + asPath + "/" + session.user._id);
			}
		} else {
	...
	const copyToClipboard = async () => {
		...
		setSuccess((prevState) => ({
			...
			message: shareableDashboardUser || shareMyFavouriteTools ? "Link has been copied to clipboard" : "Tool link has been copied to clipboard",
			showSuccessBox: true,
		}));
	};
		...
		<Modal
			...
			title={
				<div className="text-xl">
					<p>
						Supercharge your <span className="text-primary-500">productivity</span>
					</p>
					<p>
						and spread the <span className="text-primary-500">magic</span>
					</p>
					<p className="text-xs mt-3">
						Share this incredible {shareableDashboardUser || shareMyFavouriteTools ? "collection" : "tool"} with your friends and
					</p>
					<p className="text-xs">colleagues to level up your productivity together!</p>
				</div>
			}
			content={
				<div>
					...
					<div className="flex justify-between mt-8">
						...
						<Link
							...
							href={"https://twitter.com/intent/tweet?url=" + uriEncodedSharableText}
							passHref={true}
						>
						...
						<Link
							...
							rel="noopener noreferrer"
							className="flex items-center rounded-lg p-3 cursor-pointer whitespace-nowrap bg-light-200 hover:bg-gray-200"
							href={`https://web.whatsapp.com/send?text=` + uriEncodedSharableText}
							passHref={true}
						>
						...
						<Link
							...
							rel="noopener noreferrer"
							className="flex items-center rounded-lg p-3 cursor-pointer whitespace-nowrap bg-light-200 hover:bg-gray-200"
							href={`https://telegram.me/share/url?url=` + uriEncodedSharableText}
							passHref={true}
						>
