  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      <div>
        <Menu.Button
          className={
            "inline-flex w-full min-w-[125px] text-primary-500 font-semibold justify-center gap-x-1.5 pl-3 text-sm " +
            (styledButton
              ? "bg-white pr-3 py-2 rounded-md min-w-fit ring-2 ring-primary-400"
              : "")
          }
        >
          <span
                ...
                placeholder !== "Sort By" && (
                  <Menu.Item key={"i" + "-1"}>
                    {({ active }) => (
                      <a
                        className={classNames(
                          "bg-light-100 hover:bg-gray-100 text-gray-900 block px-4 py-2 text-sm cursor-pointer"
                        )}
                        onClick={() =>
                          handleOptionSelection({ _id: null, name: null })
                        }
                      >
                        {placeholder}
                      </a>
