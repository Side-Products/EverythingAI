      ...
      <div className="flex flex-col justify-between">
        {purchaseTerms.terms.map((term, index) => (
          <div
            key={index}
            className={
              "relative w-full flex flex-col space-y-3 pt-4 " +
              (index != 0 && "border-t border-dashed border-gray-300 mt-4")
            }
          >
            {index != 0 && (
        ...
        {purchaseTerms?.terms && purchaseTerms.terms.length < 4 && (
          <div className="flex items-center justify-start mt-6">
            ...
            >
              <span className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-lg -mt-[1px]">
                +
              </span>
            </button>
            <label
              htmlFor="add-term-length"
              className="pl-3 text-sm font-normal cursor-pointer"
            >
