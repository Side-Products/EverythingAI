...
export default function ToolsWithPagination({ options, resultsPerPage, totalCount, filteredTotalCount, shareableDashboardUser, shareMyFavouriteTools }) {
			...
			<div className="w-full flex flex-col items-center justify-center mt-14">
				{options && options.length === 0 ? (
					...
					<div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-12">
						{options && options.map((option) => <ToolCard key={option._id} tool={option} />)}
					</div>
				)}
			</div>
