...
const DeleteCategoryConfirmModal = ({ isOpen, setOpen, categoryToDelete }) => {
	...
	const { error, isDeleted, loading } = useSelector((state) => state.deleteCategory);
					...
					>
						<div className="flex flex-col justify-center">
							<div className="flex justify-center items-center gap-x-4">
								<p className="text-3xl font-semibold text-dark-600">{categoryToDelete.name}</p>
							</div>
						</div>

						<p className="text-sm font-semibold text-center mt-10">Are you sure you want to delete this category?</p>
						<p className="text-xs text-center mt-2">Deleting a category will also delete all its sub-categories</p>
