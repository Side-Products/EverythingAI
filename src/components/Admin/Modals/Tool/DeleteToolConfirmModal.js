...
const DeleteToolConfirmModal = ({ isOpen, setOpen, toolToDelete }) => {
	...
	const { error, isDeleted, loading } = useSelector((state) => state.deleteTool);
					...
					>
						<div className="flex flex-col justify-center items-center">
							<div className="flex justify-center items-center gap-x-4 mb-4">
								<p className="text-3xl font-semibold text-dark-600">{toolToDelete.name}</p>
							</div>

							<Image src={toolToDelete.image} alt={toolToDelete.name} width={320} height={180} className="rounded-md" />

							<p className="text-sm text-dark-600 mt-4">{toolToDelete.oneLiner}</p>
						</div>

						<p className="text-sm font-semibold text-center mt-10">Are you sure you want to delete this tool?</p>
