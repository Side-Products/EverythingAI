...
const UnverifyToolConfirmModal = ({ isOpen, setOpen, toolToUnverify }) => {
	...
	const { error, isUnverified, loading } = useSelector((state) => state.unverifyTool);
					...
					>
						<div className="flex flex-col justify-center items-center">
							<div className="flex justify-center items-center gap-x-4 mb-4">
								<p className="text-3xl font-semibold text-dark-600">{toolToUnverify.name}</p>
							</div>

							<Image src={toolToUnverify.image} alt={toolToUnverify.name} width={320} height={180} className="rounded-md" />

							<p className="text-sm text-dark-600 mt-4">{toolToUnverify.oneLiner}</p>
						</div>

						<p className="text-sm font-semibold text-center mt-10">Are you sure you want to unverify this tool?</p>
