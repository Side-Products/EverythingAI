...
import { deleteCollection, clearErrors } from "@/redux/actions/collectionActions";
import { DELETE_COLLECTION_RESET } from "@/redux/constants/collectionConstants";
import { StatusContext } from "@/store/StatusContextProvider";
import Loader from "@/components/ui/Loader";
import { sleep } from "@/utils/Sleep";

const DeleteCollectionConfirmModal = ({ isOpen, setOpen, collectionToDelete }) => {
	const { setSuccess, setError } = useContext(StatusContext);

	const dispatch = useDispatch();
	const router = useRouter();

	const { error, isDeleted, loading } = useSelector((state) => state.deleteCollection);
					...
					>
						<div className="flex flex-col justify-center">
							<div className="flex justify-center items-center gap-x-4">
								<p className="text-3xl font-semibold text-dark-600">{collectionToDelete.name}</p>
							</div>
						</div>

						<p className="text-sm font-semibold text-center mt-10">Are you sure you want to delete this collection?</p>
