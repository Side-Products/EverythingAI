...
import { deleteAiRecommendation, clearErrors } from "@/redux/actions/aiRecommenderActions";
import { DELETE_AI_RECOMMENDATION_RESET } from "@/redux/constants/aiRecommenderConstants";
import { StatusContext } from "@/store/StatusContextProvider";
import Loader from "@/components/ui/Loader";
import { sleep } from "@/utils/Sleep";

const DeleteRecommendationConfirmModal = ({ isOpen, setOpen, recommendationToDelete }) => {
	const { setSuccess, setError } = useContext(StatusContext);

	const dispatch = useDispatch();
	const router = useRouter();

	const { error, isDeleted, loading } = useSelector((state) => state.deleteAiRecommendation);
					...
					>
						<div className="flex flex-col justify-center">
							<div className="flex flex-col justify-center items-center gap-x-4">
								<p className="text-3xl font-semibold text-dark-600">{recommendationToDelete.whatAreYouHereFor}</p>
								<p className="text-xl font-semibold text-dark-600 mt-4">{recommendationToDelete.category}</p>
								<p className="text-xl font-semibold text-dark-600">{recommendationToDelete.subCategory}</p>
								<p className="text-sm text-dark-600 mt-2">by {recommendationToDelete?.user?.email}</p>
							</div>
						</div>

						<p className="text-sm font-semibold text-center mt-10">Are you sure you want to delete this recommendation entry?</p>
