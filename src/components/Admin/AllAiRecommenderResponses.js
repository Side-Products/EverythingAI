...
export default function AllAiRecommendations() {
  ...
  const { recommendations, recommendationsCount, error, loading } = useSelector(
    (state) => state.allAiRecommendations
  );

  const { setError } = useContext(StatusContext);

  const [activeTable, setActiveTable] = useState("allAiRecommendationsTable");
  const [isDeleteRecommendationModalOpen, setDeleteRecommendationModalOpen] =
    useState(false);
      ...
      <div className="flex justify-between w-full">
        ...
        >
          <li
            className="nav-item"
            role="presentation"
            onClick={() => setActiveTable("allAiRecommendationsTable")}
          >
            <span
              className={
        ...
        <div className="flex flex-col items-end gap-y-4">
          <div className="font-semibold text-base text-dark-700">
            Total Count: {recommendationsCount}
          </div>
        </div>
      </div>
      ...
      <div className="tab-content" id="tabs-tabContent3">
        ...
        {activeTable == "allAiRecommendationsTable" && (
          <div
            ...
            id="tabs-all-recommendations"
            role="tabpanel"
            aria-labelledby="tabs-all-recommendations-details"
          >
                    ...
                    <tr className="bg-light-100 text-dark-700" key={index}>
                      <td className="p-3">
                        <span className="flex align-items-center">
                          <span className="ml-3">
                            <span className="">
                              {getTimestamp(recommendation.createdAt).slice(
                                0,
                                19
                              )}
                            </span>
                      ...
                      <td className="p-3">
                        <Link
                          href={`/ai-recommender/results/${recommendation._id}`}
                      ...
                      <td className="p-3">
                        {recommendation.whatAreYouHereFor}
                      </td>
                      ...
                      <td className="p-3">
                        {recommendation.helpsMeWith
                          ? recommendation.helpsMeWith
                          : "-"}
                      </td>
                      <td className="p-3">
                        {recommendation.whatsImportant.join(", ")}
                      </td>
