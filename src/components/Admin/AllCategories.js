...
export default function AllCategories() {
	...
	const { categories, categoriesCount, error, loading } = useSelector((state) => state.allCategories);

	const { setError } = useContext(StatusContext);

	const [activeTable, setActiveTable] = useState("allCategoriesTable");
	const [isUpdateCategoryModalOpen, setUpdateCategoryModalOpen] = useState(false);
		...
		<div className="grid w-full mt-10 overflow-scroll">
			<div className="flex justify-between w-full">
				<ul className="items-start nav nav-tabs flex flex-row flex-wrap list-none border-b-0 pl-0 mb-4" id="tabs-tables" role="tablist">
					<li className="nav-item" role="presentation" onClick={() => setActiveTable("allCategoriesTable")}>
						<span
							className={
								...
								(activeTable == "allCategoriesTable" && "text-primary-500 bg-primary-100")
							}
				...
				<div className="flex flex-col items-end gap-y-4">
					<div className="font-semibold text-base text-dark-700">Total Count: {categoriesCount}</div>
					<div onClick={() => setAddCategoryModalOpen(true)} className="text-sm hover:text-primary-500 transition duration-300 cursor-pointer">
			...
			<div className="tab-content" id="tabs-tabContent3">
				...
				{activeTable == "allCategoriesTable" && (
					<div className="tab-pane fade show active" id="tabs-all-categories" role="tabpanel" aria-labelledby="tabs-all-categories-details">
						{categories.length !== 0 ? (
										...
										<tr className="bg-light-100 text-dark-700" key={index}>
											<td className="p-3">
												<span className="flex align-items-center">
													<span className="ml-3">
														<span className="">{getTimestamp(category.createdAt).slice(0, 19)}</span>
													</span>
												</span>
											</td>
											<td className="p-3">
			...
			<AddCategoryModal isOpen={isAddCategoryModalOpen} setOpen={setAddCategoryModalOpen} />
			<UpdateCategoryModal isOpen={isUpdateCategoryModalOpen} setOpen={setUpdateCategoryModalOpen} categoryToUpdate={categoryToUpdate} />
			<DeleteCategoryConfirmModal isOpen={isDeleteCategoryModalOpen} setOpen={setDeleteCategoryModalOpen} categoryToDelete={categoryToDelete} />
		</div>
	) : (
