...
const AiRecommenderResults = ({ recommendation }) => {
	...
	return (
		<>
			<div>The results here are unbiased and created based on your use case.</div>
			...
			<div className="hidden lg:block w-full border-2 rounded-xl p-2 mt-8 pb-8">
				<div className="w-full grid grid-cols-11 gap-10 items-start mt-8 font-bold">
					...
					<div className="col-span-3 text-center">
						Tool 1 <span className="text-gradient-secondary-tr">(Recommended)</span>
					</div>
				...
				<div className="w-full grid grid-cols-11 gap-10 items-start mt-6">
					...
					{recommendation.tools &&
						...
						recommendation.tools.map((tool) => (
							<div className="col-span-3 flex justify-center" key={tool._id}>
        ...
        {recommendation.tools &&
          ...
          recommendation.tools.map((tool) => (
              ...
              {tool.timesVisited ? tool.timesVisited : <>-</>}
				...
				<div className="w-full grid grid-cols-11 gap-10 items-start mt-6 ml-5">
					<div className="col-span-2 flex items-end gap-2">
						<Image src="/ui/trustpilot.png" height={100} width={120} loading="lazy" />
					</div>
					{recommendation?.tools?.map((tool, i) => (
						<div key={i} className="col-span-3">
							{tool.reviews?.trustPilot?.stars ? (
								<>
									<StarRating rating={tool.reviews?.trustPilot?.stars} size={"small"} />
									<div className="flex gap-x-8 mt-2">
					...
					{recommendation?.tools?.map((tool, i) => (
						<div key={i} className="col-span-3">
							{tool.reviews?.everythingAI?.stars ? (
								<>
									<StarRating rating={tool.reviews?.everythingAI?.stars} size={"small"} />
									<div className="flex gap-x-8 mt-2">{tool.reviews?.everythingAI?.stars}/5</div>
								</>
							) : (
								<span className="text-center w-full flex justify-center">-</span>
							)}
				...
				<div className="w-full grid grid-cols-11 gap-10 items-start mt-6 ml-5">
					<div className="col-span-2 flex items-end gap-2 font-medium">Product Hunt Reviews</div>
					{recommendation?.tools?.map((tool, i) => (
						<div key={i} className="col-span-3">
							{tool.reviews?.productHunt?.stars ? (
								<>
									<StarRating rating={tool.reviews?.productHunt?.stars} size={"small"} />
									<div className="flex gap-x-8 mt-2">
				...
				<div className="w-full grid grid-cols-11 gap-10 items-start mt-12">
					<div className="col-span-2 font-bold pl-5 bg-orange-100 rounded-lg py-3">Social</div>
					{recommendation?.tools?.map((tool, i) => (
						<div key={i} className="col-span-3 h-full gap-x-4 items-center w-full flex justify-start">
							{tool.linkedin ? (
								<Link
									href={tool.linkedin}
									target="_blank"
							...
							{!tool.linkedin && !tool.twitter && !tool.instagram && !tool.youtube && <>-</>}
						</div>
					))}
				</div>
				...
				<div className="w-full grid grid-cols-11 gap-10 items-start mt-6 ml-5">
					...
					{recommendation?.tools?.map((tool, i) => (
						<div key={i} className="col-span-3 text-base whitespace-pre-line features-list pl-4">
							<ContentToList content={tool.features} />
						</div>
					))}
				</div>
				<div className="w-full grid grid-cols-11 gap-10 items-start mt-10">
					<div className="col-span-2 font-bold ml-5">Use Cases</div>
					{recommendation?.tools?.map((tool, i) => (
						<div key={i} className="col-span-3 text-base whitespace-pre-line features-list">
				...
				<div className="w-full grid grid-cols-11 gap-10 items-start mt-10">
					<div className="col-span-2 font-bold ml-5">Pricing</div>
					{recommendation?.tools?.map((tool, i) => (
				...
				<div className="w-full grid grid-cols-11 gap-10 mt-10 items-center">
					<div className="col-span-2 font-bold ml-5">Add to collection?</div>
					{recommendation?.tools?.map((tool, i) => (
						<div key={i} className="col-span-3 flex justify-center">
							<Button
								variant={"classic"}
				...
				<div className="w-full grid grid-cols-11 gap-10 items-start mt-10">
					...
					{recommendation?.tools?.map((tool, i) => (
						<div key={i} className="col-span-3 flex justify-center">
							<Link href={`/tools/${tool?.slug}`} target="_blank" rel="noopener noreferrer" className="w-full">
								<Button type={"button"}>
									Check out tool <i className="ml-2 text-xs fas fa-external-link-alt" />
								</Button>
							</Link>
						</div>
					))}
			...
			<div className="lg:hidden block w-full border-2 rounded-xl p-2 px-6 mt-8 pb-8">
				<MobileViewTool tool={recommendation?.tools && recommendation?.tools[0]} position={1} />
				<MobileViewTool tool={recommendation?.tools && recommendation?.tools[1]} position={2} />
				<MobileViewTool tool={recommendation?.tools && recommendation?.tools[2]} position={3} />
			</div>
			...
			<div className="w-full mt-20">
				<ToolsCarousel tools={recommendation.similarTools} heading={"Similar Tools"} />
			</div>
		</>
	);
};
