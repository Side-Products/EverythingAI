...
import { createAiRecommendation, clearErrors } from "@/redux/actions/aiRecommenderActions";
...
const AiRecommender = () => {
	...
	const uniqueCategories = Array.from(new Set(categories?.map((category) => category.name))).map((name) => {
		return categories?.find((category) => category.name === name);
	});

	const { subcategories } = useSelector((state) => state.allSubCategories);
	const uniqueSubCategories = Array.from(new Set(subcategories?.map((subcategory) => subcategory.name))).map((name) => {
	...
	const submitForm = () => {
		if (whatAreYouHereFor == "Explore how I can use AI") {
			...
			if (session && session.user) {
				...
				const cat = iAmInOptions.find((category) => category.name.toLowerCase() === document.getElementById("category").value.toLowerCase());
	...
	const { success, recommendation, error } = useSelector((state) => state.createAiRecommendation);
			...
			<div className="w-full lg:w-2/3 md:grid md:grid-cols-2 flex flex-col mt-10 py-16 md:px-32 sm:px-16 px-8 bg-white rounded-2xl md:gap-8 gap-2 md:items-end">
				...
				<div>
					<Dropdown
						...
						options={["Find AI tools", "Explore how I can use AI", "Want to list my AI tool here"]}
						required={true}
				...
				{whatAreYouHereFor == "Find AI tools" && (
					<>
						...
						<div>
							<Dropdown variant="secondary" id={"workFor"} name="workFor" options={["Startup", "SMB", "Enterprise"]} required={true} />
						</div>
						...
						<div>
							<Dropdown
								...
								objKey={"name"}
								setChoice={(e) => {
									const cat = iAmInOptions.find((category) => category.name.toLowerCase() === e.target.value.toLowerCase());
									const realCat = getObjectByName(cat.category, uniqueCategories);
						...
						<div className="flex h-full items-start text-start md:mt-0 mt-6">That helps me with</div>
						<div>
							<Textarea
								variant="secondary"
								id={"helpsMeWith"}
					...
					<div>
						<Button variant={"primary"} classes="text-md px-4 py-3 group mt-4">
							<span className="transition-all duration-500 group-hover:pr-6">
								{whatAreYouHereFor == "Find AI tools" ? "Give me the Top 3 Tools" : "Submit"}
