			...
			<section aria-label="Pricing Section">
				<div className="grid gap-6 md:grid-cols-3 items-start max-w-6xl px-4 mx-auto py-6">
					{terms.map((term) => (
						<>
							<div className="border border-zinc-200 p-6 rounded-lg">
								<h2 className="font-bold text-2xl mb-2">{term?.termLength} Plan</h2>
								<p className="text-sm mb-4">{onLiner(term?.termLength)}</p>
								<div className="line-through text-gray-500 mb-2">${term?.actualPrice}</div>
								<div className="text-2xl font-bold mb-4">${term?.discountedPrice}</div>
								<Button>Buy Now</Button>{" "}
								<div className="mt-2">
									<Tooltip labelText="Terms & Conditions*" theme="glass" message={term?.termsAndConditions} />
								</div>
							</div>
						</>
					))}
