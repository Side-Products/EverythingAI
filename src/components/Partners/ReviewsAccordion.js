...
export default function ReviewsAccordion({ reviewFromCompanies, size = "large" }) {
	const [currentlyExpanded, setCurrentlyExpanded] = useState(0);

	const accordion = reviewFromCompanies?.map((elem, idx) => {
		return (
			<div
				key={idx}
				className={
					(idx == 0
						...
						: idx == reviewFromCompanies.length - 1
						? "rounded-b-xl border-[2px] border-t-0"
						: "border-[2px] border-t-0") +
					(reviewFromCompanies.length == 1 ? " rounded-b-xl border-[2px]" : "") +
					" p-4 border-gray-300"
				}
			>
				<h2 className="mb-0" id={elem.companyName}>
					<button
						type="button"
						className={
							...
							(size == "small" ? " py-1 pb-3 px-2 text-md" : " py-4 px-5 text-xl")
						}
						onClick={() => {
				...
				{/* Item Content */}
				<div
					id={elem.companyName + "target"}
					className={"!visible " + (currentlyExpanded != idx && "hidden")}
					// className={"!visible hidden"}
					data-te-collapse-item
					// data-te-collapse-show={currentlyExpanded == idx ? "true" : "false"}
					aria-labelledby={elem.companyName}
					// data-te-parent="#accordionExample"
				>
					<div className={size == "small" ? " py-1 px-2 text-sm" : " py-4 px-5 text-xl"}>{elem.review}</div>
				</div>
			</div>
		);
	});
