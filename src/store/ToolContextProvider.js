...
function ToolContextProvider({ children }) {
  ...
  const {
    error: createLikedToolError,
    success,
    likedTool,
  } = useSelector((state) => state.createLikedTool);
  const { error: deleteLikedToolError, isDeleted } = useSelector(
    (state) => state.deleteLikedTool
  );
  useEffect(() => {
  ...
  const [toolToRemoveFromCollection, setToolToRemoveFromCollection] =
    useState(null);
  const [collectionToRemoveFrom, setCollectionToRemoveFrom] = useState(null);
  const [
    isRemoveToolFromCollectionModalOpen,
